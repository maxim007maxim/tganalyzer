...
def classify_niche_llm(description: str, title: str, username: str, posts_text: str) -> str | None:
    ...
    if not GROQ_API_KEY:
        return None
    prompt = (
        "Ты классификатор Telegram-каналов. Определи нишу канала ТОЧНО по описанию и постам.\n\n"
        "НИШИ (ключ: что сюда входит):\n"
        "- arbitrage: арбитраж трафика, CPA, партнёрки, медиабаинг, лидогенерация, офферы, вебмастера, click/traffic\n"
        "- crypto: криптовалюты, Bitcoin, Web3, NFT, DeFi, блокчейн, альткоины, крипто-трейдинг\n"
        "- finance: инвестиции, фондовый рынок, акции, личные финансы, бюджет, ипотека (НЕ крипто)\n"
        "- business: бизнес, предпринимательство, стартапы, маркетплейсы (WB, Ozon), B2B, продажи\n"
        "- it: программирование, разработка, DevOps, IT-технологии, software, код, техника\n"
        "- education: обучение, курсы, образование, школы, университеты, саморазвитие\n"
        "- lifestyle: лайфстайл, красота, мода, здоровье, фитнес, питание, путешествия\n"
        "- sport: спорт, футбол, баскетбол, хоккей, тренировки, матчи, беттинг\n"
        "- gaming: видеоигры, гейминг, стриминг, киберспорт, Twitch\n"
        "- news: новости, политика, события, СМИ, аналитика происходящего\n"
        "- entertainment: юмор, мемы, развлечения, приколы, шоу, досуг\n"
        "- default: не подходит ни под одну из категорий выше\n\n"
        "ПРАВИЛА:\n"
        "- Крипто-трейдинг → crypto, не finance\n"
        "- Арбитраж трафика/CPA → arbitrage, даже если упоминаются деньги\n"
        "- При выборе между двумя — смотри на ОСНОВНУЮ тему канала\n"
        "- Отвечай ТОЛЬКО ключом из списка, без объяснений\n\n"
        f"Название: {title}\n"
        f"Username: @{username}\n"
